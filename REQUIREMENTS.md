...
* **FR-CAR-01 (Canonical Coordinate Hierarchy & Locomotion)**: Vehicle architecture adheres to standard $-Z$ forward, $+Z$ rear, $+Y$ up hierarchy. Imported GLTF visual meshes normalized under `VisualRoot` (`rotation_degrees.y = 180.0`). Headlights anchored at $Z = -1.82$ facing $-Z$, rear taillights at $Z = 1.76$ and dual rocket thrusters at $Z = 1.88$ facing $+Z$, and front bumper Area3D at $Z = -1.90$ facing $-Z$. Features responsive vehicular steering with dynamic front-wheel yaw steering ($\pm 28^\circ$), active boost flame scaling, ground drift (1.75x steer multiplier), throttle acceleration, reversing, emergency braking, double-jump vertical thrusters (`can_double_jump`), and aerial pitch/yaw/roll orientation with local transforms.
* **FR-CAR-02 (Authentic Vehicle Archetypes & Physics Stats)**: 4 production rocket-sports vehicle archetypes in `MeshBuilder`: *Apex Spectre* (Sports Coupe), *Dune Raider* (Rally Buggy), *Titan Enforcer* (Muscle GT), and *Volt Pulse* (Futuristic EV). Each features authentic physical handling: mass (1050–1400 kg), acceleration (28–36 m/s²), boost top speed (42–52 m/s), aerial agility, and dynamic collider reconfiguration via `set_vehicle_archetype()`.
* **FR-CAR-03 (Ball Physics & Anti-Tunneling)**: Physics-driven soccer ball with single-impulse `apply_central_impulse` integration in `apply_ball_impulse` (eliminating double-velocity glitches), `reset_ball()` API alias, continuous collision detection (CCD) with 4-contact reporting, spin momentum transfer, and restitution bouncing.
* **FR-CAR-04 (Regulation Stadium, Net Cages & Crowd)**: Symmetrical regulation sports colosseum ($110\text{m} \times 64\text{m} \times 20\text{m}$) with $45^\circ$ octagonal containment corners, $2.5\text{m}$ lower kickboards with scrolling LED ribbons, transparent acrylic upper panels, regulation 3D goal cages ($16\text{m} \times 6.5\text{m} \times 6.0\text{m}$) with visible white post frames and net geometry, 28 turf boost pads + 6 full-boost orbs with dynamic respawn timers, MultiMesh crowd system with dynamic states (`idle`, `goal`, `celebration`), and correct goal attribution (North goal scores for Blue, South goal scores for Orange).
* **FR-CAR-05 (AI Roles, Lead Intercept & Aerials)**: Dynamic team role allocation (`STRIKER`, `SUPPORT`, `DEFENDER`, `GOALKEEPER`) for 1v1, 2v2, 3v3 matches, predictive lead intercept calculation (`predict_ball_intercept`) with iterative time-of-flight convergence, aerial header jumps for balls at $y \in [2.5\text{m}, 6.0\text{m}]$, kickoff sprint boost utilization, and angular roll recovery.

### 3.4 Game 4: Drift Storm (Arcade Kart Racing)
* **FR-KART-01 (4-Wheel Suspension & Grounding Dynamics)**: 4-wheel downward raycast suspension system (`SuspensionRay_0..3`) at $(\pm 0.42, 0.12, \pm 0.58)$ with spring compression damping, rolling wheel rotation ($\omega = v/r$), front wheel steering yaw ($\pm 28^\circ$), chassis rest at $y \in [0.0, 0.25\text{m}]$ with $0.0\text{m}$ hovering, tight arcade steering, lateral slip drift locking, 3-tier drift charging with chromatic sparks, and post-drift boost.
* **FR-KART-02 (6 Global Circuits & Authority Model)**: Single authoritative `RaceSpline` driving track generation, collision, AI navigation, and lap tracking across 6 distinct global circuits: Volt Speedway ($755\text{m}$), Sunset Coast ($830\text{m}$), Canyon Run ($848\text{m}$), Skyline Drift ($812\text{m}$), Alpine Rush ($865\text{m}$), and Storm Harbor ($820\text{m}$). Counter-clockwise road quad winding with explicit `Vector3.UP` normals and double-sided colliders.
