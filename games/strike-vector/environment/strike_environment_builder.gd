...
static func _build_urban_segment(root: Node3D, seg_idx: int, length: float, width: float) -> void:
	...
	# 1. Hardened Continuous Roadway Floor (Authentic 14m wide 2-lane street + parking)
	var road_len = length + (16.0 if seg_idx == 0 else 4.0)
	var z_offset = -length * 0.5 + (6.0 if seg_idx == 0 else -1.0)
	var road_w = 14.0
	var sidewalk_w = 3.5
	var total_street_w = road_w + sidewalk_w * 2.0 # 21.0m canyon width

	_create_solid_floor(root, Vector3(road_w, 1.2, road_len), Vector3(0, -0.6, z_offset), mat_road)

	# 2. Elevated Grimy Concrete Sidewalks & Curbs (3.5m wide)
	...
	# 5. Authored 3D Modular Buildings (Dark Dystopian Blackout Architecture: 16m-24m tall)
	var building_types = [
	...
	var b_dist_x = road_w * 0.5 + sidewalk_w + 6.0 # 7.0 + 3.5 + 6.0 = 16.5m from center
	for z_step in range(0, int(length), 14):
		...
		# Believable skyscraper facade heights: 18m, 24m, 20m, 16m
		var h_left = 18.0 if (z_step % 28 == 0) else 24.0
		var h_right = 22.0 if (z_step % 28 == 0) else 16.0

		# Left Building with dark facade override and unscaled physical collision
		_add_authored_building(root, b_path, Vector3(-b_dist_x, 0.0, z_pos), Vector3(12.0, h_left, 12.0), 90.0)
		# Right Building with dark facade override and unscaled physical collision
		_add_authored_building(root, b_path_r, Vector3(b_dist_x, 0.0, z_pos), Vector3(12.0, h_right, 12.0), -90.0)

		# Streetlights on sidewalk curb edge: blackout effect (most unpowered, selective emergency lights)
		var light_z = z_pos + 3.0
		var is_emergency = (z_step % 28 == 0)
		_add_streetlight(root, Vector3(-road_w * 0.5 - 0.5, 0.0, light_z), Color(1.0, 0.72, 0.35), is_emergency)
		_add_streetlight(root, Vector3(road_w * 0.5 + 0.5, 0.0, light_z + 7.0), Color(0.95, 0.65, 0.25), not is_emergency and (z_step % 14 == 0))
		...
		if z_step % 28 == 0:
			...
			if not bb:
				_create_visual_box(root, Vector3(0.3, 2.5, 6.0), Vector3(-road_w * 0.5 - sidewalk_w, 7.5, z_pos), mat_dark)

	# 6. Segment-Specific Tactical Dressing & Believable Metric Vehicles
	match seg_idx:
		0:
			# Zone 0: Deployment Roadblock & Heavy Transport Truck (6.16m long)
			_add_model(root, "res://assets/models/vehicles/truck_yellow.glb", Vector3(-3.5, 0.0, -12.0), Vector3(2.2, 2.2, 2.2), 15.0)
			_add_model(root, "res://assets/models/environment/scifi/barrier_high.glb", Vector3(3.5, 0.0, -18.0), Vector3(1.8, 1.8, 1.8), 0.0)
			_add_beacon(root, Vector3(3.5, 1.5, -18.0), Color(1.0, 0.45, 0.0))
		...
		1:
			# Zone 1: Damaged Intersection: Abandoned Police Enforcer (4.56m car), Barricades & Hazard Beacons
			_add_model(root, "res://assets/models/vehicles/rocket_car_enforcer.glb", Vector3(3.5, 0.0, -14.0), Vector3(1.6, 1.6, 1.6), -45.0)
			_add_model(root, "res://assets/models/environment/scifi/barrier_high.glb", Vector3(-3.5, 0.0, -22.0), Vector3(1.8, 1.8, 1.8), 20.0)
			_add_beacon(root, Vector3(-3.5, 1.5, -22.0), Color(1.0, 0.45, 0.0))
			_add_model(root, "res://assets/models/environment/scifi/pipe_network.glb", Vector3(-road_w * 0.5 - 1.5, 0.0, -16.0), Vector3(2.0, 2.0, 2.0), 0.0)
		2:
			# Zone 2: Checkpoint & Fortified Gate: Heavy Transport Truck (6.38m long) & Roadblock
			_add_model(root, "res://assets/models/vehicles/truck_green.glb", Vector3(3.8, 0.0, -15.0), Vector3(2.2, 2.2, 2.2), 170.0)
			_add_model(root, "res://assets/models/environment/scifi/wall_pillar.glb", Vector3(-4.5, 0.0, -10.0), Vector3(2.0, 2.0, 2.0), 0.0)
			_add_model(root, "res://assets/models/environment/scifi/wall_pillar.glb", Vector3(4.5, 0.0, -10.0), Vector3(2.0, 2.0, 2.0), 0.0)
			_add_model(root, "res://assets/models/environment/scifi/barrier_high.glb", Vector3(0.0, 0.0, -10.0), Vector3(1.8, 1.8, 1.8), 0.0)
			_add_beacon(root, Vector3(0.0, 1.5, -10.0), Color(1.0, 0.2, 0.1))
		3:
			# Zone 3: Alley & Warehouse Loading Dock: Utility Truck (6.16m), Civilian Vehicle (4.52m), Generator
			_add_model(root, "res://assets/models/vehicles/truck_red.glb", Vector3(-3.8, 0.0, -16.0), Vector3(2.2, 2.2, 2.2), 30.0)
			_add_model(root, "res://assets/models/vehicles/rocket_car_spectre.glb", Vector3(3.8, 0.0, -20.0), Vector3(1.7, 1.7, 1.7), -135.0)
			...
			_add_model(root, "res://assets/models/environment/scifi/pipe_network.glb", Vector3(road_w * 0.5 + 1.5, 0.0, -18.0), Vector3(2.2, 2.2, 2.2), 180.0)
			_add_model(root, "res://assets/models/environment/scifi/computer_terminal.glb", Vector3(3.5, 0.2, -8.0), Vector3(1.3, 1.3, 1.3), 0.0)
		4:
			...
			_add_model(root, "res://assets/models/environment/scifi/barrier_high.glb", Vector3(-3.5, 0.0, -14.0), Vector3(1.8, 1.8, 1.8), -15.0)
			_add_beacon(root, Vector3(-3.5, 1.5, -14.0), Color(1.0, 0.45, 0.0))
			_add_model(root, "res://assets/models/environment/scifi/barrier_high.glb", Vector3(3.5, 0.0, -26.0), Vector3(1.8, 1.8, 1.8), 15.0)
		5:
			...
			if jammer:
				...
				j_light.omni_range = 14.0
				j_light.position = Vector3(0, 2.5, 0)
				jammer.add_child(j_light)
			_add_model(root, "res://assets/models/environment/scifi/barrier_high.glb", Vector3(-4.0, 0.0, -length * 0.5 + 4.0), Vector3(1.8, 1.8, 1.8), 45.0)
			_add_model(root, "res://assets/models/environment/scifi/barrier_high.glb", Vector3(4.0, 0.0, -length * 0.5 + 4.0), Vector3(1.8, 1.8, 1.8), -45.0)
		_:
			# Zone 6+: Evacuation Plaza & Extraction Zone
			_add_model(root, "res://assets/models/environment/scifi/barrier_high.glb", Vector3(-4.5, 0.0, -15.0), Vector3(1.8, 1.8, 1.8), 45.0)
			_add_model(root, "res://assets/models/environment/scifi/barrier_high.glb", Vector3(4.5, 0.0, -15.0), Vector3(1.8, 1.8, 1.8), -45.0)
			_add_beacon(root, Vector3(0.0, 0.3, -length * 0.5), Color(0.2, 1.0, 0.4))
...
static func _add_authored_building(parent: Node3D, path: String, pos: Vector3, sc: Vector3, rot_y: float) -> Node3D:
	...
	if m:
		var dark_facade = MaterialGenerator.get_material("grimy_concrete")
		_apply_dark_building_facade(m, dark_facade)
	...
	box.size = Vector3(12.0, sc.y, 12.0)
	col.shape = box
	sb.add_child(col)
	parent.add_child(sb)
	if m:
	...
	var mat_hull = MaterialGenerator.get_material("dark_hull")
	return _create_solid_floor(parent, Vector3(12.0, sc.y, 12.0), pos + Vector3(0, sc.y * 0.5, 0), mat_hull)

static func _apply_dark_building_facade(node: Node, mat: Material) -> void:
	if not node:
		return
	if node is MeshInstance3D:
		node.material_override = mat
	for child in node.get_children():
		_apply_dark_building_facade(child, mat)

static func _add_streetlight(parent: Node3D, pos: Vector3, col: Color, is_emergency_active: bool = false) -> void:
	...
	# Urban Blackout: most streetlights are unpowered; selective emergency illumination
